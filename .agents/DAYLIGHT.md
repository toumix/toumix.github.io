...
- each session is assigned an issue (an existing one, or one written for USER to file) or a
  PR, sometimes several: that is the focus — the day file and its comments give the context of
  what the other agents are doing
- it picks up 🐦 Birdsong's plan and designs it with USER; an ambiguous instruction is a
  question, not a task
- feedback that stays within the assigned work lives on its PR; only feedback that goes outside
